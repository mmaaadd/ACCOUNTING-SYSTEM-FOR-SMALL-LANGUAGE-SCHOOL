<script> // select proper menu item as active
	...
	<tbody>
		...
		<tr>
			...
			<td class="form_cell">
				<input class = "form-control" name="kupa" id="opis" type="text" >
			</td>
		</tr>
		<tr>
			<td class="form_cell_desc">
		...
		<tr>
			...
			<td class="form_cell">
                <input class="btn btn-primary" onclick="saveCostSam()" style="width:350px" value='Dodaj'>
	...
	<?php
				...
				$st='<table id="tablica_koszty_sam" class="display" > <thead> <tr><th width="100px"><p><b>Data</b></p></th><th width="100px"><p><b>Nazwa sklepu</b> </p></th><th width="100px"><p> <b>Nr rach.</b> </p></th><th width="100px"><p> <b>Opis</b></p></th><th width="100px"><p> <b>Kwota</b></p></th><th width="100px">&nbsp</th></tr></thead></table>';
				echo $st;
		?>

<script>

//function to save cost connected with car:
    function saveCostSam() {
        var checkbox_val;    
        if($("#sprzet:checked").val()==1){//get checkbox value
            checkbox_val=1}
        else {
            checkbox_val=0}    
        postParameters=({data : $("#SelectedDate1").val(), sklep:$("#sklep").val(), ndk:$("#ndk").val(), opis:$("#opis").val(),koszt:$("#koszt").val(),sprzet:checkbox_val,n:21, sam:1})  
        saveData("<p>Zapisuję koszt</p>", postParameters,refreshTable)    
    }
    
  function refreshTable() {
        $("#tablica_koszty_sam").DataTable().ajax.reload();
    }
    

	$(document).ready(function() {
		$('#tablica_koszty_sam').DataTable({
			"order": [[ 0, "desc" ]],
			"serverSide": true,
			"ajax": "api/get_car_costs_list.php"
		});
